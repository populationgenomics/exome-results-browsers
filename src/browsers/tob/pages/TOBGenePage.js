...
const TOBGenePage = () => {
  ...
  const onAssociationSelect = useCallback(
    (associations) => {
      ...
      setSelectedGeneIds(vids)
    },
    [selectedGeneIds, selectedAssociations]
  )
  ...
  const effectGridRows = useMemo(() => {
    return selectedGeneIds.map((vid) => {
      return {
        key: vid,
        help: null,
        content: vid,
        onClear: () => {
          setSelectedGeneIds(selectedGeneIds.filter((v) => v !== vid))
    ...
    <div ref={ref} style={{ width: '100%' }}>
      <LoadingOverlay active={isLoading}>
        ...
        <section>
          ...
          <p>This plot shows all genes associated with {query.gene}</p>
          <TOBAggregatePlot
            query={query.gene}
